...
// 투두 시트 정렬 규칙 (Figma 댓글 #59~#64) — 루틴(반복) 항목 맨 위 → 수기(당일 직접 추가) 항목 그 아래,
// 각 묶음 안은 최근 추가가 상단(recency). 이 규칙은 확정(accepted as-is)이며 테스트 이름이 그 규칙을 서술한다.
describe('todoList 정렬 — 루틴 항목 최상단 > 수기 당일 항목 > 묶음 내 최근 추가 순', () => {
  ...
  it('1차: 루틴 항목이 수기(당일) 항목보다 위 / 2차: 같은 묶음 안은 최근 추가(addedAt 큰 것)가 먼저', () => {
    const sorted = sortTodos([routineA, manualOld, routineB, manualNew])
    expect(sorted.map(t => t.id)).toEqual(['routine-2', 'routine-1', 'todo-2', 'todo-1'])
  })
  ...
  it('수기 추가(addManualTodo)는 같은 규칙으로 정렬된 배열을 돌려주고 빈 텍스트·상한 초과는 null', () => {
    ...
    expect(added?.map(t => t.text)).toEqual(['청소', '독서'])
  ...
  it('루틴 프리필(mergeRoutineTodos)은 중복 routineId 를 건너뛰고 상한까지만 채우며, 수기 항목 위에 놓인다', () => {
    const merged = mergeRoutineTodos([routineA, manualNew], [{ id: 1, label: '청소' }, { id: 3, label: '피부과' }], 400)
    expect(merged.map(t => t.id)).toEqual(['routine-3', 'routine-1', 'todo-2'])
